...
function generatePassword() {
  ...
  var userInputLength = prompt(
    "Your password must contain minimum 8 maximum 128 characters. Enter a number."
  );
  ...
  while (
    userInputLength < 8 ||
    userInputLength > 128 ||
    isNaN(parseInt(userInputLength)) === true
  ) {
    alert(
      " That was an invalid entry! Please enter a number value between 8 and 128!"
    );
  ...
  var userInputLowerCaseLetters = confirm(
  ...
  var userInputUpperCaseLetters = confirm(
  ...
  var userInputNumbers = confirm(
    "Would you like your password to include numeric values?"
  );
  ...
  if (
    userInputLowerCaseLetters === false &&
    userInputUpperCaseLetters === false &&
    userInputNumbers === false &&
    userInputSymbols === false
  ) {
    alert(
      "Your password must contain at least ONE type of character. Please try it again."
    );
    return generatePassword();
  }
  ...
  for (var i = 0; i < userInputLength; i++) {
    ...
    // inspect the loop with debugger
    //debugger;
    password =
      password +
